...
public abstract class AbstractStoredEntityService<T extends AbstractStoredEntity<T>> {
  ...
  public ImmutableList<T> getAll() {
    TypedQuery<T> createQuery =
        (TypedQuery<T>) getEntityManager().createQuery("from " + getEntityName(), getEntityClass());
  ...
  public SolrServerWrapper getSolrServer() {
    if (solrserver == null) {
      solrserver =
          new RemoteSolrServer(
              Configuration.instance().getSetting(Configuration.SOLR_URL_KEY),
              new ElaborateEditorQueryComposer());
    }
    return solrserver;
  }
  ...
  void checkEntityFound(T entity, long id) {
    if (entity == null) {
      ...
      throw new NotFoundException(
          MessageFormat.format("No {0} found with id {1,number,#}", getEntityName(), id));
    }
  }
  ...
  EntityManager getEntityManager() {
    ...
    if (em == null) {
      throw new RuntimeException(
          "no entityManager set, did you call openEntityManager() or beginTransaction()?");
    }
    return em;
  }
  ...
  private void checkProjectWritePermissions(long project_id, User user) {
    ...
    if (user.getPermissionFor(project).canWrite()) {
      ...
      throw new UnauthorizedException(
          "user "
              + user.getUsername()
              + " has no write permission for project "
              + project.getName());
    }
  }
