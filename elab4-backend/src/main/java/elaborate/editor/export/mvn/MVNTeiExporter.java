...
public class MVNTeiExporter {
  ...
  // opening annotations
  private void handleOpeningAnnotations(
      StringBuilder teiBuilder, AnnotatedTextSegment annotatedTextSegment) {
    Multimap<String, XmlAnnotation> openingAnnotationIndex =
        indexXmlAnnotations(annotatedTextSegment.getOpeningAnnotations());
    for (AnnotationHandler annotationHandler : ANNOTATION_HANDLERS) {
      List<XmlAnnotation> xmlAnnotations =
          filterRelevantAnnotations(openingAnnotationIndex, annotationHandler);
      annotationHandler.onOpenAnnotation(teiBuilder, xmlAnnotations, context);
    }
  }

  // closing annotations
  private void handleClosingAnnotations(
      StringBuilder teiBuilder, AnnotatedTextSegment annotatedTextSegment) {
    Multimap<String, XmlAnnotation> closingAnnotationIndex =
        indexXmlAnnotations(annotatedTextSegment.getClosingAnnotations());
    for (AnnotationHandler annotationHandler : Lists.reverse(ANNOTATION_HANDLERS)) {
      List<XmlAnnotation> xmlAnnotations =
          filterRelevantAnnotations(closingAnnotationIndex, annotationHandler);
      annotationHandler.onCloseAnnotation(teiBuilder, xmlAnnotations, context);
    }
  }

  private List<XmlAnnotation> filterRelevantAnnotations(
      Multimap<String, XmlAnnotation> closingAnnotationIndex, AnnotationHandler annotationHandler) {
  ...
  private static class TekstHandler extends DefaultAnnotationHandler {
    ...
    public void onOpenAnnotation(
      ...
      for (XmlAnnotation xmlAnnotation : xmlAnnotations) {
        ...
        if (attributes.containsKey("title")) {
          ...
          teiBuilder
              .append(openingTag(head))
              .append(attributes.get("title"))
              .append(closingTag(head));
        }
      }
    }
    ...
    public void onCloseAnnotation(
        StringBuilder teiBuilder, Collection<XmlAnnotation> collection, Context context) {
  ...
  private static class AlineaHandler extends ElementWrapper {
    ...
    //    @Override
    //    public void onCloseAnnotation(StringBuilder teiBuilder, Collection<XmlAnnotation>
    // xmlAnnotations, Context context) {
    //      if (!xmlAnnotations.isEmpty()) {
    //        context.inParagraph = false;
    //      }
    //      super.onCloseAnnotation(teiBuilder, xmlAnnotations, context);
  ...
  private static void openHeadOrCloser(
      StringBuilder teiBuilder,
      Collection<XmlAnnotation> xmlAnnotations,
      String name,
      Context context) {
    context.inPoetry = false;
  ...
  private static void closeHeadOrCloser(
      StringBuilder teiBuilder,
      Collection<XmlAnnotation> xmlAnnotations,
      String name,
      Context context) {
  ...
  private static class VersregelHandler extends DefaultAnnotationHandler {
    ...
    public void onOpenAnnotation(
      ...
      if (!xmlAnnotations.isEmpty()) {
        if (!context.inPoetry) {
          context.addError(
              MVNAnnotationType.VERSREGEL, "versregel annotatie buiten een mvn:poetry sectie");
  ...
  private static class LHandler extends DefaultAnnotationHandler {
    ...
    public void onOpenAnnotation(
      ...
      if (!xmlAnnotations.isEmpty()) {
        ...
        Element lb =
            new Element("lb")
                .withAttribute("n", String.valueOf(context.foliumLineNumber))
                .withAttribute(XML_ID, id);
        if (context.indent) {
  ...
  private static void addOptionalAttribute(
      Element pb, String key, Map<String, String> entryAttributes) {
