...
public class ElaborateSolrIndexerTest extends AbstractTest {
  ...
  // ImmutableList.<ProjectEntryMetadataItem> of();
  // User creator = new User();
  // entry = project.addEntry("brief 1",
  // creator).setProjectEntryMetadataItems(projectEntryMetadataItems);
  ...
  public void test_newlines_in_entry_metadata_are_replaced() {
    ...
    SolrInputDocument docForEditor =
        ElaborateSolrIndexer.getSolrInputDocument(mockEntry, false, null);
    ...
    assertThat(docForEditor.getField(SolrFields.PROJECT_ID).getValue())
        .isEqualTo(mockEntry.getProject().getId());
    assertThat(docForEditor.getField(SolrFields.PUBLISHABLE).getValue())
        .isEqualTo(mockEntry.isPublishable());
    assertThat(docForEditor.getField("metadata_multiline").getValue())
        .isEqualTo("Metadata/values/with/multiple/lines");
    ...
    assertThat(docForPublication.getField(SolrFields.NAME).getValue())
        .isEqualTo(mockEntry.getName());
  ...
  public void test_multivalued_metadata_is_split_into_multiple_fields() {
    ...
    Project mockProject = mock(Project.class);
    when(mockProject.getId()).thenReturn(2L);
    when(mockProject.getProjectEntryMetadataFieldnames())
        .thenReturn(ImmutableList.of("Field 1", "MultiField 1"));
    ...
    assertThat(docForEditor.getField(SolrFields.PUBLISHABLE).getValue())
        .isEqualTo(mockEntry.isPublishable());
    assertThat(docForEditor.getField("metadata_multifield_1").getValue())
        .isEqualTo("Value 1 | Value 2 | Value 3");

    Collection<String> multiValuedFacetNames = ImmutableList.of("metadata_multifield_1");
    SolrInputDocument docForPublication =
        ElaborateSolrIndexer.getSolrInputDocument(mockEntry, true, multiValuedFacetNames);
    assertThat(docForPublication != null).isTrue();
    Log.info("docForPublication={}", docForPublication);
    assertThat(docForPublication.getField(SolrFields.ID).getValue()).isEqualTo(mockEntry.getId());
    assertThat(docForPublication.getField(SolrFields.NAME).getValue())
    ...
    assertThat(docForPublication.getField(SolrFields.PROJECT_ID)).isEqualTo(null);
    assertThat(docForPublication.getField(SolrFields.PUBLISHABLE)).isEqualTo(null);
    assertThat(docForPublication.getField("mv_metadata_multifield_1").getValues())
        .containsOnly("Value 1", "Value 2", "Value 3");
  }
  ...
  public void testExtractCorrespondentsFromValueWithSlash() {
    ...
    assertThat(extractCorrespondents)
        .containsExactly("Boddaert, Elisabeth Carolina", "Spengler, Constantia Gerharda Heije van");
  }
  ...
  public void testExtractCorrespondentsFromValueWithArrow() {
    ...
    List<String> extractCorrespondents = ElaborateSolrIndexer.extractCorrespondents(value);
    assertThat(extractCorrespondents)
        .containsExactly("Alberdingk Thijm, Karel Joan Lodewijk", "Verwey, Albert");
  }

  @Test
  public void testExtractCorrespondentsFromValueWithSlashedAndArrow() {
    String value =
        "Groesbeek, Klaas/Nijhoff, Paulus-->Verwey, Albert/Alberdingk Thijm, Karel Joan Lodewijk";
    List<String> extractCorrespondents = ElaborateSolrIndexer.extractCorrespondents(value);
    assertThat(extractCorrespondents)
        .containsExactly(
            "Groesbeek, Klaas",
